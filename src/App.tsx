...
const partnerGroups: Array<{ id: string; title: string; featured?: boolean; wide?: boolean; partners: Partner[] }> = [
  ...
  },
  {
    ...
    wide: true,
    partners: [
      { name: "Polimaster", href: "https://polimaster.com/" },
      { name: "Ачинта" },
      { name: "ЭкоСфера", href: "https://ekosf.ru/" },
      { name: "Экотест", href: "https://ecotest.ua/" },
      { name: "ПТФМ", href: "https://www.ptfm.ru/" },
      { name: "НПП «Гамма»", href: "https://khv.nppgamma.ru/" },
      { name: "Атомтех", href: "http://www.atomtex.com/" },
      { name: "Позитрон", href: "https://npk-positron.ru/" },
      { name: "IBA Dosimetry", href: "https://www.iba-dosimetry.com/" },
      { name: "Gammex", href: "https://www.sunnuclear.com/diagnostic-imaging-qa-solutions" },
    ],
  },
];
...
const requisites = [
  ["Почтовый адрес", "124498, Москва, а/я 169, ООО «Техноцентр»"],
  ["ИНН", "7735506935"],
  ["КПП", "504401001"],
  ["ОГРН", "1047796715889"],
  ["ОКПО", "74651115"],
    ...
    <>
      <header className={`site-header ${compact ? "is-compact" : ""}`}>
        <div className="container header-inner">
          <a className="brand" href="#top" aria-label="Техноцентр" onClick={() => setMenuOpen(false)}>
...
function PartnerLogo({ partner }: { partner: Partner }) {
  const content = partner.logo ? <img src={partner.logo} alt={partner.name} /> : partner.name;
  const className = `partner-logo ${partner.logo ? "image-logo" : "text-logo"} ${partner.className ?? ""}`;
